...
// Entrada: arredonda para cima até ao próximo múltiplo de 15
// Se já é múltiplo de 15, mantém
...
// Saída: arredonda para baixo até ao múltiplo de 15 anterior
// Se já é múltiplo de 15, mantém
...
function calcHours(e) {
  ...
  const extraMinutes = Math.max(0, total - 480);
...
function manualEntry() {
  ...
  const entry = { date: getToday(), start, lunchOut, lunchIn, end };
  calcHours(entry);
  ...
  last7.forEach((e) => {
    ...
    div.innerHTML += `
      <div class="list-item">
        ...
        <span>
          <button class="secondary" onclick="openEdit(${realIdx})">✏️</button>
          <button class="secondary" onclick="deleteEntry(${realIdx})">🗑️</button>
        </span>
      </div>
    `;
  });
