function addExpense() {
  ...
  const custom = document.getElementById("expenseName").value.trim();
  const value = parseFloat(document.getElementById("expenseValue").value);

  if (!value) return;

  // Nome final da despesa
  let name = "";

  if (preset === "outros") {
    if (!custom) return; // se escolheu outros, tem de escrever
    name = custom;
  } else if (preset) {
    name = preset;
    if (custom) name += " — " + custom; // descrição opcional
  } else {
    if (!custom) return;
    name = custom;
  }
